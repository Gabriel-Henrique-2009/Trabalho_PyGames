...
vida_teste_5 = 0
###
mask_tiro_boss = pygame.mask.from_surface(imagem_atk_chatgpt)
...
while rodando:
    ...
    if estado_jogo == "MENU":
        ...
        if disparo:
            ...
            armazenar_tiro_boss.append({"rect": novo_tiro,"dire": direcao, "mask": mask_tiro_boss})

      
        for tiro_b in armazenar_tiro_boss[:]:
            tiro_b["rect"].x += velocidade_tiro_boss * tiro_b["dire"]

            # Remove se sair da tela
            if tiro_b["rect"].right < 0 or tiro_b["rect"].left > 1200:
                armazenar_tiro_boss.remove(tiro_b)
                continue

            # Colisão com o Jogador
            off_x = tiro_b["rect"].x - alexandre.rect.x
            off_y = tiro_b["rect"].y - alexandre.rect.y
            if alexandre.mask.overlap(tiro_b["mask"], (off_x, off_y)):
                if time.time() - tempo_imunidade_player > 1.0:
                    vida_player -= 1
                    tempo_imunidade_player = time.time()
                    armazenar_tiro_boss.remove(tiro_b)
                    print(f"VC tomou um tiro da IA sigma 🤣🤣")

        # --- 2. LOGICA DOS TIROS DO JOGADOR (CONTRA O BOSS) ---
        for tiro_p in armazenar_tiro[:]:
            tiro_p["rect"].x += velocidade_tiro * tiro_p["dire"]

            # Remove se sair da tela
            if tiro_p["rect"].right < 0 or tiro_p["rect"].left > 1200:
                armazenar_tiro.remove(tiro_p)
                continue

            # Colisão com o Boss
            mask_tiro_player = pygame.mask.from_surface(imagem_tiro)
            off_boss_x = tiro_p["rect"].x - robo.rect.x
            off_boss_y = tiro_p["rect"].y - robo.rect.y
            if robo.mask.overlap(mask_tiro_player, (off_boss_x, off_boss_y)):
                vida_teste_5 += 1  # Dano no Boss
                armazenar_tiro.remove(tiro_p)
                print(f"Atingiu o robo. Vida: {vida_teste_5}/{vida_boss_max}")
                print(f"VC tomou um tiro da IA sigma 🤣🤣")
